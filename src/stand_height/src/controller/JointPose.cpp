...
JointPose JointPose::fromLegState(const go2_utils::interact::lowlevel::Leg::SharedPtr &leg) {
    JointPose jp;
    fromLegState(jp, leg);
    return jp;
}

void JointPose::fromLegState(JointPose &jp, const go2_utils::interact::lowlevel::Leg::SharedPtr &leg) {
    jp.hip_q = leg->hip()->state().q;
    jp.thigh_q = leg->thigh()->state().q;
    jp.calf_q = leg->calf()->state().q;
}

void JointPose::toLegCommand(JointPose &jp, const go2_utils::interact::lowlevel::Leg::SharedPtr &leg) {
