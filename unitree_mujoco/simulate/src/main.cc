                     ...
                     mj::Simulate::Clock::now() - startCPU < Seconds(refreshTime))
              {
                ...
                if (param::config.enable_elastic_band == 1)
                {
                  if (elastic_band.enable_)
                  {
                    ...
                    int body_id = param::config.band_attached_link;
                    int adr = 6 * body_id;

                    // safety check
                    if (adr + 5 >= 6 * m->nbody || adr < 0) {
                      std::cerr << "[ERROR] Invalid xfrc_applied index!" << std::endl;
                    } else {
                      // clear previous wrench (IMPORTANT)
                      for (int i = 0; i < 6; i++) {
                        d->xfrc_applied[adr + i] = 0.0;
                      }

                      // apply force (fx, fy, fz)
                      d->xfrc_applied[adr + 0] = elastic_band.f_[0];
                      d->xfrc_applied[adr + 1] = elastic_band.f_[1];
                      d->xfrc_applied[adr + 2] = elastic_band.f_[2];
                    }
                  }
                }
...
if (body_id < 0) {
  std::cerr << "[FATAL] Cannot find torso_link or base_link in model!" << std::endl;
  exit(1);
}

std::cout << "[DEBUG] Elastic band attached to body id: " << body_id << std::endl;
// store BODY ID (not multiplied)
param::config.band_attached_link = body_id;
